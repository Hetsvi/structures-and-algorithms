...
public class Display extends JFrame {
    ...
    public class GraphPanel extends JPanel {
        ...
        public void paintGraph(Graphics g) {
            ...
            int number = 255;
            for(Vertex key: graph.getVertices()){  // loops through the vertices
                for(Edge edges : key.getListEdge()){    // gets edge on the list of vertices
                    paintEdge(g, edges.getSource(),  edges.getTarget(), edges.getDistance(),
                            Color.LIGHT_GRAY, DEFAULT_THICKNESS, number);   // uses paintEdge
                }
            }

            // this makes the city names
            // TODO: for each vertex in the graph, use paintVertex(g, v); to print the vertex
            for(Vertex key :  graph.getVertices()){   // loops vertices
                paintVertex(g, key);                  // paintVertex to print
            }
